...
```{r}
anki_cards <- read_tsv("anki_char_notes.tsv", locale = readr::locale(encoding="UTF-8"), col_names = FALSE)[, c(1,2)]
...
```{r}
anki_cards_test <- anki_cards %>% filter(!grepl("\\(", raw_char)) %>% filter(!grepl("<br>", raw_char))

meaning <- lapply(strsplit(anki_cards_test$descr, " "), 
       function(entry) {
           entry[3]
       }) %>% unlist() %>% 
    gsub("<br>.*", "", .) %>% 
    gsub("<div>.*", "", .) %>%
    gsub(",.*", "", .)
anki_cards_test$char <- anki_cards_test$raw_char %>% gsub("[ (].*", "", .)

# write_tsv(anki_cards_test, path = "C:/Users/jakob/Desktop/anki_check_2.tsv")
# head(anki_cards_test, 100)
